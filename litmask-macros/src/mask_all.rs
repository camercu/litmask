...
/// Macro-classification logic. Pure function over `syn::Macro` —
/// no walker state, no side effects. Lives in a submodule so its
/// surface is unit-testable in isolation: the walker calls
/// `classify::classify_macro` once per macro invocation, but a test
/// can construct arbitrary `syn::Macro` values and assert the family
/// directly.
mod classify {
    use proc_macro2::TokenStream as TokenStream2;
